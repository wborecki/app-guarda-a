  ...
  useEffect(() => {
    const fetchPublished = async () => {
      ...
      const mapped = data.map((s, i) => {
        ...
        const hostRate = (s as any).price_per_day || MIN_DAILY_RATE;
        ...
        return {
          ...
          area: Number(vol) || 8,
          pricePerDay: hostRate,
          price_per_day: hostRate,
  ...
  const filteredSortedSpaces = useMemo(() => {
    ...
    if (filters.maxPrice !== null) {
      result = result.filter(s => {
        ...
        const bp = calculatePrice(Math.max(totalVol, 1), effectiveDays, rate);
        return bp.total <= filters.maxPrice!;
      });
    }
    ...
    switch (sortBy) {
      ...
      case "price_asc": result.sort((a, b) => (a.pricePerDay || MIN_DAILY_RATE) - (b.pricePerDay || MIN_DAILY_RATE)); break;
    ...
    return result;
  }, [allSpaces, sortBy, filters, effectiveDays, totalVol]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col overflow-x-hidden">
        ...
        <div className="container py-2 sm:py-2.5">
          <div className="flex items-center gap-2 sm:gap-x-4 text-xs sm:text-sm overflow-x-auto scrollbar-hide overscroll-x-contain">
            ...
            {deliveryDate && pickupDate ? (
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <Calendar size={12} className="text-primary flex-shrink-0" />
                <span className="text-muted-foreground">{effectiveDays} {effectiveDays === 1 ? "dia" : "dias"}</span>
              </div>
            )}
            <button
      ...
      <div className="flex-1 flex flex-col lg:flex-row">
        ...
        <div className={`lg:flex-1 lg:sticky lg:top-[105px] lg:h-[calc(100vh-105px)] ${mobileView === "list" ? "hidden lg:block" : "flex-1"}`}>
          <Suspense fallback={<MapSkeleton />}>
            <SpaceMap spaces={mapSpaces} highlightedId={highlightedSpaceId} onPinHover={handlePinHover} onPinClick={handlePinClick} className="h-full w-full lg:rounded-none" />
          </Suspense>
        </div>
      </div>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-30 lg:hidden pb-[env(safe-area-inset-bottom)]">
        ...
        >
          {mobileView === "list" ? <><MapIcon size={16} /> Ver mapa</> : <><List size={16} /> Ver lista</>}
        </button>
      </div>

      <MobileFilterDrawer open={mobileFiltersOpen} onClose={() => setMobileFiltersOpen(false)} filters={filters} setFilters={setFilters} clearAll={clearAll} resultCount={filteredSortedSpaces.length} />
    </div>
  );
};
